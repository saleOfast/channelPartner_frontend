...
const ManageUsersTable = ({ start, end, deleteConfirm, disableConfirm, leadList, openEdtMdl, title, setShowAssignTo, oldAssignTo, setoldAssignTo, setShowDateFilter, getDataList, loader, maxDate, cpId, setCpId, statusId, setStatusId }) => {
  ...
  const columns = [
    {
      ...
      options: {
        ...
        customHeadRender: (columnMeta, updateDirection) => (
          <th style={{ background: `${clientBtnColor}`, backgroundColor: 'rgb(133, 64, 232)', color: 'white', paddingLeft: "15px", padding: "8px" }}   >
            {columnMeta.label}
          </th>
        ),
    ...
    {
      name: 'Lead Valid upto',
      label: "Lead Valid upto",
      options: {
        filter: false,
        customHeadRender: (columnMeta, updateDirection) => (
          <th style={{ background: `${clientBtnColor}`, backgroundColor: 'rgb(133, 64, 232)', color: 'white', paddingLeft: "15px", padding: "8px" }}   >
  ...
  const tableHeaderStyles = {
    "& .MuiTableCell-head": {
      backgroundColor: "rgb(133, 64, 232)",
      color: "#fff",
      fontWeight: "bold",
    },
  };
