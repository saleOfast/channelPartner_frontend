...
import SidebarSkelton from "../../Basics/SidebarSkelton";
...
const SideBarDMSWeb = ({}) => {
  ...
  const getSidebarInfo = async (navLink) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${Baseurl}/db/permission/${navLink}?pf=MEDIA`, header);
              ...
              <div className={`sideMenuBox ${currActiveLink === allais_menu ? "open" : "closed"}`}>
                <div className="header">
                  <div className="icon" title={allais_menu}>
                    {
                      dbMode=="user" ? 
                      // <img src={`/sidebarIcons/${icon_path ? icon_path : `shield.svg`}`} alt="logo" width="100%" /> 
                      <img src={`/sidebarIcons/shield.svg`} alt="logo" width="100%" /> 
                      : <img src={`/sidebarIcons/shield.svg`} alt="logo" width="100%" />
                    }
                ...
                <div className="sidebar-sublist">
                  <ul className="sublists">
                    {children?.map((item) => (
                      <Link  href={`${dbMode !== 'user' ?  `/${item.link}` : `/media/${item.link}`} `} key={item.menu_id}>
