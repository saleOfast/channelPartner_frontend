...
const DynamicTable = dynamic(
  () => import('./PendingApprovalManagementTable'),
  { ssr: false }
)

export default function PendingApprovalManagementScreen() {
    ...
    <>
      ...
      <div className={`main_Box  ${sideView}`}>
        <div className="bread_head">
          <h3 className="content_head">PENDING APPROVALS</h3>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb">
              <li className="breadcrumb-item"> <Link href='/dms'>Home   </Link></li>
              <li className="breadcrumb-item active" aria-current="page">Pending Approvals List</li>
            </ol>
          </nav>
        </div>
