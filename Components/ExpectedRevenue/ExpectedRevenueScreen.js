...
const DynamicTable = dynamic(() => import("./ExpectedRevenueTable"), {
  ssr: false,
});
import Select from "react-select";

const ExpectedRevenueScreen = () => {
  ...
  const handleDownload = async () => {
    if (hasCookie("token")) {
      ...
      const fileName = `Expected_Weighted_Revenue.xlsx`;
    ...
    <>
      <div className={`main_Box  ${sideView}`}>
        <div className="bread_head">
          <h3 className="content_head">EXPECTED (WEIGHTED) REVENUE (FY)</h3>
...
<Col xs={12} sm={6} md={4} lg={2} className="d-flex justify-content-end align-items-end w-100">
      ...
      <DynamicTable
        title="Expected (Weighted) Revenue List"
        dataList={dataList}
        loader={loader}
      />
        </Container>
...
export default ExpectedRevenueScreen;
