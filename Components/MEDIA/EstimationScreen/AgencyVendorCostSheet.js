...
const AgencyVendorCostSheet = ({id}) => {
  ...
  const getAgencySites = async (id) => {
    if (hasCookie('token')) {
        ...
        try {
            const response = await axios.get(Baseurl +`/db/media/costSheet/vendorCostSheet/getAgencyCostSheetsData?estimate_id=${id}`, header);
...
export default AgencyVendorCostSheet