...
const AssetClientCostSheet = ({id}) => {
  ...
  const getAssetSites = async (id) => {
    if (hasCookie('token')) {
        ...
        try {
            const response = await axios.get(Baseurl + `/db/media/costSheet/clientCostSheet/getCostSheetsData?estimate_id=${id}`, header);
...
export default AssetClientCostSheet