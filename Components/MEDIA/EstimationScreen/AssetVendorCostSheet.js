...
const AssetVendorCostSheet = ({id}) => {

  const [assetSiteLists, setAssetSiteLists] = useState([]);

  const getAssetSites = async (id) => {
    if (hasCookie('token')) {
        ...
        try {
            const response = await axios.get(Baseurl +`/db/media/costSheet/vendorCostSheet/getCostSheetsData?estimate_id=${id}`, header);
...
<div className="add_screen_head">
                    <span className="text_bold">Vendor Cost Sheet</span>
                  </div>
...
export default AssetVendorCostSheet