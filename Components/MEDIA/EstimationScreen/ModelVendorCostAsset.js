...
const ModelVendorCostAsset = ({
  ...
  async function getAssetSites1() {
    await fetchData(
      `/db/media/costSheet/vendorCostSheet/getCostSheetsData?estimate_id=${estimateId}`,
      setAssetSiteLists,
      errorToast,
      setErrorToast
    );
  ...
  const getAssetSites = async () => {
    if (hasCookie('token')) {
        ...
        try {
            const response = await axios.get(Baseurl +`/db/media/costSheet/vendorCostSheet/getCostSheetsData?estimate_id=${estimateId}`, header);
