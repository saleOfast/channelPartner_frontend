...
const DynamicTable = dynamic(
  () => import('./ManageEstimationStatusTab'),
  { ssr: false }
)

const ManageEstimationStatusScreen = () => {
  const sideView = useSelector((state) => state.sideView.value);

  const [show, setShow] = useState(false);
  const [dataList, setDataList] = useState([])
  const [userInfo, setUserInfo] = useState({ est_s_name: '' })
  const [editMode, setEditMode] = useState(false)
  const [disableShowConfirm, setdisableShowConfirm] = useState(false)
  const [deleteshowConfirm, setdeleteshowConfirm] = useState(false)
  const [currObj, setcurrObj] = useState({ est_s_id: "", action: "" });
  ...
  const openEdtMdl = (value) => {
    setEditMode(true)
    setUserInfo({ ...userInfo, est_s_name: value[0], est_s_code: value[1], est_s_id: value[3] })
    handleShow();
  }
  ...
  function disableConfirm(value, type) {
    ...
    setcurrObj({ est_s_id: value, action: type });
    ...
    <>
      ...
      <div className={`main_Box  ${sideView}`}>
        <div className="bread_head">
          <h3 className="content_head">ESTIMATION STATUS MASTER</h3>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb">
              <li className="breadcrumb-item">
                <Link href="/setting">Home </Link>
              </li>
              <li className="breadcrumb-item"> Estimation Status Master</li>
            </ol>
          </nav>
        </div>
      ...
      <Modal className="commonModal" show={show} onHide={handleClose} >
        <Modal.Header closeButton>
          <Modal.Title> {editMode ? 'EDIT' : ' ADD'} ESTIMATION STATUS</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <div className="row">
              <div className="col-xl-12 col-md-12 col-sm-12 col-12">
                <div className="input_box">
                  <label htmlFor="email">Estimation Status Name</label>
                  <input
                    type="text"
                    placeholder='Enter Estimation Status Name'
                    name="email" id="email"
                    className="form-control"
                    onChange={(e) => setUserInfo({ ...userInfo, est_s_name: e.target.value })}
                    value={userInfo.est_s_name ? userInfo.est_s_name : ''}
                  />
...
export default ManageEstimationStatusScreen;
