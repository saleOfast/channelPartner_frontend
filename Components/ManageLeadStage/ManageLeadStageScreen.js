...
const ManageLeadStageScreen = () => {
  ...
  const [userInfo, setUserInfo] = useState({ stage_name: "" });
  ...
  function disableConfirm(value, type) {
    ...
    setcurrObj({ lead_stg_id: value, action: type });
  ...
  async function disableHandler() {
    const reqInfo = {
      lead_stg_id: currObj.lead_stg_id,
    ...
    if (hasCookie("token")) {
      ...
      try {
        const response = await axios.delete(
          Baseurl + `/db/leadstg?stg_id=${currObj}`,
          header
        );
        if (response.status === 204 || response.status === 200) {
          toast.success(response.data.message);
  ...
  const updateHandler = async () => {
    if (userInfo.stage_name == "") {
      toast.error("Please enter the Stage");
    } else {
      if (hasCookie("token")) {
        let token = getCookie("token");
        let db_name = getCookie("db_name");
    ...
    <>
      ...
      <Modal className="commonModal" show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title> {editMode ? "EDIT" : " ADD"} LEAD STAGE</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <div className="row">
              <div className="col-xl-12 col-md-12 col-sm-12 col-12">
                <div className="input_box">
                  <label htmlFor="email">Lead Stage Name</label>
                  <input
                    type="text"
                    ...
                    onChange={(e) =>
                      setUserInfo({ ...userInfo, stage_name: e.target.value })
                    }
                    value={userInfo.stage_name ? userInfo.stage_name : ""}
                  />
                </div>
              </div>
            </div>
...
export default ManageLeadStageScreen;
