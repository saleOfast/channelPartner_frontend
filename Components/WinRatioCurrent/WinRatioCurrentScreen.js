...
const DynamicTable = dynamic(() => import("./WinRatioCurrentTable"), {
  ssr: false,
});
import Select from "react-select";

const WinRatioCurrentScreen = () => {
    ...
    if (hasCookie("token")) {
      ...
      try {
        const response = await axios.get(
          Baseurl + `/db/opportunity/getOpportunityRatio?${queryString} `,
          header
        );
  ...
  const handleDownload = async () => {
    if (hasCookie("token")) {
      ...
      const fileName = `Win_Ratio_Curent.xlsx`;
    ...
    <>
      <div className={`main_Box  ${sideView}`}>
        <div className="bread_head">
          <h3 className="content_head">WIN RATIO CURRENT FY</h3>
...
export default WinRatioCurrentScreen;
